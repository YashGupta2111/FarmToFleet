...
const updatedatacarefully=async(item)=>{
  ...
  let result=await fetch(`http://localhost:7134/updatedetails/${currentCustomer._id}`,{
   method:'PUT',
...
{
let result= await fetch(`http://localhost:7134/getcustomerdetails/${detail.email}`);
result=await result.json();
